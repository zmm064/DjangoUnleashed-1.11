...
class ActivationMailFormMixin:
    ...
    def log_mail_error(self, **kwargs):
        ...
        for recipient in recipient_list:
            msg_list.insert(1, f'recipient: {recipient}\n')
        if 'error' in kwargs:
            level = ERROR
    ...
    def _send_mail(self, request, user, **kwargs):
        ...
        try:
            # the number of emails that was sent
            ...
            if number_sent > 0:
                return (True, None)  # 保持一致与出错的情况返回同类型数据
        # we don’t raise an exception and that we don’t send an email
        self.log_mail_error(**mail_kwargs)
        return (False, 'unknownerror')

    def send_mail(self, user, **kwargs):
        '''
        That's what developers will actually call to send an email.
        '''
